	...
	{//lightsaber
		...
		{//already have it up, let's try to toggle it
			...
			{//player can't activate/deactivate saber when in a cinematic
				...
				if ( cg.predicted_player_state.stats[STAT_HEALTH] > 0 && (!cg_entities[0].gent->client->ps.saberInFlight || g_entities[cg_entities[0].gent->client->ps.saberEntityNum].s.pos.trType == TR_LINEAR ) )
